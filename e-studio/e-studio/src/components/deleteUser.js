...
const DeleteUser = (props) => {
    ...
    const validate = (e) => {
        axios.get('http://localhost:8080/show', { headers: { "token": localStorage.getItem("token"), "select_value": email } }).then(function (response) {
            ...
            fetch('http://localhost:8080/deactivated_user', requestOptions).then(response => response.json()).then(data => {
                document.getElementById("success").textContent = "Utilisateur "+email+" desactiver avec succes";
                console.log(data);
